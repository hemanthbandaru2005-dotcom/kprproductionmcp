      ...
      <div id="photography-section" className="w-full bg-white border-b border-[#E2D9CC] overflow-hidden transition-all duration-500">
        ...
        {isExpanded && (
          ...
          <div className="w-full bg-[#F7F3EE] border-b border-[#E2D9CC] px-3 sm:px-8 py-2.5 sm:py-3.5 flex items-center justify-center">
            <div className="inline-flex justify-center items-center gap-1 sm:gap-2 p-1 bg-white border border-[#E2D9CC] rounded-full shadow-sm max-w-full overflow-x-auto scrollbar-none">
              {/* Gallery Tab */}
              <button
                onClick={() => setActiveTab('gallery')}
                className={`inline-flex justify-center items-center gap-1.5 sm:gap-2 px-4 sm:px-8 py-2 sm:py-2.5 text-[11px] sm:text-xs font-bold tracking-widest uppercase rounded-full transition-all duration-300 cursor-pointer shrink-0 ${
                  activeTab === 'gallery'
              ...
              <button
                ...
                className={`inline-flex justify-center items-center gap-1.5 sm:gap-2 px-4 sm:px-8 py-2 sm:py-2.5 text-[11px] sm:text-xs font-bold tracking-widest uppercase rounded-full transition-all duration-300 cursor-pointer shrink-0 ${
                  activeTab === 'estimator'
                    ? 'bg-[#C5A880] text-black shadow-md font-black'
                    : 'text-[#555555] hover:text-[#1A1A1A] hover:bg-[#F7F3EE]'
                }`}
              >
                <Sparkles className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'estimator' ? 'text-black' : 'text-[#C5A880]'}`} />
